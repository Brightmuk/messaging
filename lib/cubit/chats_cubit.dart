...
class ChatsCubit extends Cubit<ChatsState> {
  ...
  Future<void> loadChats({bool isInitialLoad = true}) async {
    ...
    final bool shouldReset = isInitialLoad ||
        (state is ChatsLoaded &&
            (state as ChatsLoaded).isDefaultApp != isDefault);
...
// Remove or update chat when messages are deleted
  Future<void> _handleMessageDeleted(String threadId) async {
    ...
    final remaining =
        await _smsService.getMessagesForThread(threadId, limit: 1);
  ...
  Future<void> requestDefaultRole() async {
    ...
    _count = 0; // reset count on each request
    _defaultAppTimer =
  ...
  Future<void> _syncMissedMessages() async {
    try {
      // Find the newest message we already have
      final latestDate = await _smsService.getLatestMessageDate();
      if (latestDate == 0) {
        // No messages at all — do a full sync
        await _smsService.syncExistingMessages();
        return;
      }
      // Only fetch what we missed
      await _smsService.syncMessagesSince(latestDate);
    } catch (_) {}
  }
